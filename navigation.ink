...
+ {can_navigate(parentsroom)} [Enter room 203.]
    ~currentlocation = parentsroom
+ {can_navigate(ghostroom)} [Enter room 213.]
    ~currentlocation = ghostroom
+ {can_navigate(lobby)}[Enter the lobby.]
    ~currentlocation = lobby
+ {can_navigate(restaurant)}[Enter the restaurant.]
...
{ currentlocation:
    ...
    - giftshop:
        The giftshop is small, but packed full of various trinkets.
...
{currentlocation ? kiosk && saw_burns_intro && not give_burns_gloves: <-get_gloves}
